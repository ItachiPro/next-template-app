...
export const PermissionService = {
  async getPermissions({ params, headers }: GetParams) {
    const response = await api.get('/permissions', { params, headers })
    return response
  },

  async getPermission({ id, headers }: GetByIdParams) {
    const response = await api.get(`/permission/${id}`, { headers })
    return response
  },

  async savePermission({ data, headers }: SaveParams<PermissionDTO>) {
    const response = await api.post('/permission', data, { headers })
    return response
  },

  async updatePermission({ id, data, headers }: UpdateParams<PermissionDTO>) {
    const response = await api.put(`/permission/${id}`, data, { headers })
    return response
  },
